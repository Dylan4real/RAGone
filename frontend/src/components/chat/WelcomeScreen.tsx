...
export function WelcomeScreen() {
  ...
  return (
    <div className="relative flex min-h-full items-center justify-center overflow-hidden px-4 py-16 sm:px-6">
      <div
        aria-hidden="true"
        className="pointer-events-none absolute inset-0 bg-gradient-to-br from-canvas via-canvas to-surface-soft"
      />
      <div
        aria-hidden="true"
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -top-32 right-[-40px] h-72 w-72 rounded-full bg-gradient-radial from-claude-teal/20 via-transparent to-transparent blur-3xl animate-float"
      />
      <div
        aria-hidden="true"
        className="pointer-events-none absolute -bottom-36 left-[-80px] h-80 w-80 rounded-full bg-gradient-radial from-claude-amber/30 via-transparent to-transparent blur-3xl animate-float"
      />
      ...
      <div className="relative w-full max-w-[860px]">
        ...
        >
          <span className="inline-flex items-center gap-2 rounded-full border border-hairline bg-canvas/70 px-3 py-1 text-xs font-medium text-coral shadow-sm">
        ...
        >
          <div
            className={cn(
              "relative flex flex-col rounded-3xl border border-hairline/70 bg-canvas/80 px-5 pt-4 pb-3 shadow-soft backdrop-blur-xl transition-all duration-200",
              isFocused
          ...
          >
            <div className="relative">
              <textarea
                ...
                className="max-h-40 min-h-[52px] w-full resize-none border-0 bg-transparent px-2 pt-2 pb-2 text-[15px] text-ink placeholder:text-muted-claude focus:outline-none sm:text-base"
                rows={1}
          ...
          <p className="mt-3 text-center text-xs text-muted-claude">
            <kbd className="rounded bg-surface-card/80 px-1.5 py-0.5 text-body shadow-sm">
              Enter
            </kbd>{" "}
            发送
        ...
        >
          <div className="flex items-center justify-center gap-2 text-xs uppercase tracking-[0.24em] text-muted-claude">
            ...
            {promptPresets.map((preset) => {
              ...
              return (
                <button
                  ...
                  disabled={isStreaming}
                  className={cn(
                    "group rounded-2xl border border-hairline/70 bg-canvas/70 p-4 text-left shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-cream-strong hover:shadow-md",
                    isStreaming && "cursor-not-allowed opacity-60"
                  )}
                >
                  <div className="flex items-center gap-3">
                    <span className="flex h-10 w-10 items-center justify-center rounded-full bg-surface-card text-coral">
                  ...
                  <div className="mt-3 flex items-center gap-2 text-xs text-muted-claude">
                    <span className="min-w-0 flex-1 truncate">推荐问法：{preset.prompt}</span>
                    <ArrowUpRight className="h-3.5 w-3.5 text-muted-soft transition-colors group-hover:text-coral" />
                  </div>
                </button>
              );
            })}

    ...
    <div className="group flex">
      <div className="min-w-0 flex-1 space-y-4">
        ...
        {!isThinking && hasThinking ? (
          <div className="overflow-hidden rounded-xl border border-[#E5E5E5] bg-[#F9F9F9]">
            <button
              type="button"
              onClick={() => setThinkingExpanded((prev) => !prev)}
              className="flex w-full items-center gap-2 px-4 py-3 text-left transition-colors hover:bg-[#F5F5F5]"
            >
              <div className="flex flex-1 items-center gap-2">
                <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-white">
                  <Brain className="h-4 w-4 text-[#5D5D5D]" />
                </div>
                <span className="text-sm font-medium text-[#5D5D5D]">深度思考</span>
                {thinkingDuration ? (
                  <span className="rounded-full bg-white px-2 py-0.5 text-xs text-[#8F8F8F]">
                    {thinkingDuration}
                  </span>
                ) : null}
              </div>
              <ChevronDown
                className={cn(
                  "h-4 w-4 text-[#8F8F8F] transition-transform",
                  thinkingExpanded && "rotate-180"
                )}
              />
            </button>
            {thinkingExpanded ? (
              <div className="border-t border-[#E5E5E5] px-4 pb-4">
                <div className="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-[#5D5D5D]">
                  {message.thinking}
                </div>
              </div>
            ) : null}
